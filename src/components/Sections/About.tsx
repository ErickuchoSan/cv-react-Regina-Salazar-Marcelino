...
// DRY + SoC: About usa datos centralizados

export const About: React.FC = () => (
    <section id="sobre-mi" className="py-20 relative overflow-hidden" aria-labelledby="about-title">
        <div className="max-w-6xl mx-auto px-4 relative z-10">
            ...
            <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-start">
                ...
                <div className="grid gap-6">
                    {/* Educación - caso especial con estructura diferente */}
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                {EDUCATION.map((edu, index) => (
                                    <div key={index} className={index > 0 ? 'pt-2 border-t border-stone-100' : ''}>
                                        <p className="font-semibold text-stone-700">{edu.degree}</p>
                                        <p className={edu.status === 'En curso' ? 'text-teal-600 text-sm' : 'text-stone-500 text-sm'}>
                    ...
                    {INFO_CARDS.map((card, index) => (
                        <article key={index} className="card-soft p-6 flex items-start">
                            <div className={`${card.iconBgClass} p-3 rounded-full mr-4 ${card.iconColorClass}`}>
                                <card.icon className="text-xl" aria-hidden="true" />
                            </div>
                            <div>
                                <h4 className="text-lg font-bold text-stone-800 mb-2">{card.title}</h4>
                                <p className="text-stone-600">
                                    {card.title === 'Disponibilidad' ? (
                                        <>
                                            <strong>Tiempo completo</strong> • Presencial/Híbrido<br />
                                            <span className="text-sm text-stone-500">{card.subContent}</span>
                                        </>
                                    ) : card.title === 'Voluntariado' ? (
                                        <span className="text-sm">
                                            {card.content}<br />
                                            <span className="text-teal-600">{card.subContent}</span>
                                        </span>
                                    ) : (
                                        card.content
                                    )}
